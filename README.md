...
Folder `data/` jest ignorowany przez Git poza plikiem `.gitkeep`, żeby przypadkowo nie opublikować prywatnych danych.

## Oczekiwany format CSV

CSV powinien używać separatora `;` i zawierać przynajmniej kolumny:
...
Dodatkowe kolumny, które aplikacja wykorzystuje, jeśli są dostępne:
...
- kolumny powodów zwrotu w formacie `Return reason: ... (%)`

## Publikacja

Najprostsza ścieżka publikacji to GitHub + Streamlit Community Cloud:

1. Utwórz nowe repozytorium na GitHubie.
2. Wypchnij kod aplikacji do repozytorium.
3. Wejdź na Streamlit Community Cloud i wybierz repozytorium.
4. Jako plik startowy ustaw `app.py`.
5. Po wdrożeniu użytkownicy mogą wgrywać CSV przez panel boczny.

Więcej szczegółów jest w pliku [DEPLOYMENT.md](DEPLOYMENT.md).

## Prywatność

Nie wrzucaj do publicznego repozytorium prawdziwych plików CSV, cache, logów ani sekretów. `.gitignore` jest ustawiony tak, żeby pomagać tego pilnować.
